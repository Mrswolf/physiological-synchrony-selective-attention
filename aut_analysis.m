...
% filepath = '../data/autonomic';
% d = dir([filepath, '/*.set']);
% isub = [d(:).isdir] ==  false;
% filename = {d(isub).name}';
% filename(ismember(filename,{'.','..'})) = [];
% idc = contains({d.name}, 'ibi') | contains({d.name}, 'eda');
% filename(idc) = [];
% N = length(filename);
% 
% load('Stimuli')
...
stimulusIdc{1} = floor(ALLEEG(1).event(4).latency * mwa_fs/ALLEEG(1).srate) + 60*mwa_fs : ceil(ALLEEG(1).event(end-1).latency* mwa_fs/ALLEEG(1).srate);
for stim = 1 : 2
...
for j = 1 : ALLEEG(1).nbchan
    for i = 1 : length(stimulusIdc)
        ...
        for g = 1 : 2
            fprintf('   %s: %s\n    t(%i) = %f, p = %f\n', groupName{g}, string(p_isc(g,j,i) < .05),  stats_isc(g,j,i).df, stats_isc(g,j,i).tstat, p_isc(g,j,i))
        end
